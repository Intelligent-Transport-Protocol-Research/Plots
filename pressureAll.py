...
def visualize_cpu_memory_io_from_csv(file_path):
    """
    Visualizes CPU, Memory, and IO avg10 and avg60 from a CSV file.
    ...
    try:
        ...
        # Extract avg10 and avg60 values for IO
        io_avg10_some = io_data[1].str.split('=', expand=True)[1].astype(float)
        io_avg60_some = io_data[2].str.split('=', expand=True)[1].astype(float)
        # io_avg10_full = io_data[5].str.split('=', expand=True)[1].astype(float)
        # io_avg60_full = io_data[6].str.split('=', expand=True)[1].astype(float)
        ...
        # Plot IO avg10 and avg60
        plt.figure(figsize=(10, 5))
        plt.plot(seconds, io_avg10_some, label='IO some avg10')
        plt.plot(seconds, io_avg60_some, label='IO some avg60')
        # plt.plot(seconds, io_avg10_full, label='IO full avg10')
        # plt.plot(seconds, io_avg60_full, label='IO full avg60')
        plt.xlabel('Time (seconds)')
        plt.ylabel('IO Average')
        plt.title('IO avg10 and avg60 Over Time')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(output_prefix_io)
        plt.close()

        print(f"Visualization complete. Saved as {output_prefix_cpu}, {output_prefix_mem}, and {output_prefix_io}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        visualize_cpu_memory_io_from_csv(file_path)